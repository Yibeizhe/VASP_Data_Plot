...
class Chgcar():
    ...
    def chg_total(self):
        #Write the total charge density part of CHGCAR to chg_total.dat file.
        with open('chg_total.dat','w') as total_chg:
            for i in range(self.fft_line+1,self.fft_line+1+self.charge_lines):
                total_chg.write(self.chgcar[i])
        # Read the total charge density part from chg_total.dat
        # to an array whose shape is NG(X,Y,Z)
        chg_density=np.genfromtxt('chg_total.dat').reshape(self.fft)
        return chg_density
    ...
    def aver_pl_cd(self):
        average_plane_cd=np.empty(self.fft[2])
