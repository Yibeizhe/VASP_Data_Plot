...
class Dosplot():
    ...
    # Plot the dx, dy, ...,dx2
    def dxyz_plot(self):
        labels=['dxy', 'dyz','dz2','dxz','dx2']
        for i in range(5,10):
            plt.plot(self.energy,self.dos_data[:,i],label=labels[i-5],
                     linewidth=0.5,color=self.colors[i-5])
...
if __name__=="__main__":
    ...
    # plt.plot(Ag.energy, Ag.dos_frame['dz2']/2.0, color="#99C6FF", linewidth=1, label='$Ag-d_{z^2}$')
    ...
    # plt.title('Ag_FE1', fontsize=10)
    plt.tick_params(direction='in', labelsize=5)
    plt.xlim(-6, 2)
    plt.ylim(0, 20)
    plt.savefig('CIPS-FE1_CuAgS_pd', figsize=(3, 4), dpi=300, bbox_inches='tight')
    plt.show()