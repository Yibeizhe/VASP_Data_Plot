...
def metal_spd(metal):
    m_dw = Dosplot('tetra-{}N2/PDOS_{}_DW.dat'.format(metal,metal))
    m_dw.s_plot()
    m_dw.p_plot()
    m_dw.d_plot()
    m_dw.tot_plot(label='')
    m_up = Dosplot('tetra-{}N2/PDOS_{}_UP.dat'.format(metal, metal))
    m_up.s_plot()
    m_up.p_plot()
    m_up.d_plot()
    m_up.tot_plot(label='${}_{total}$'.format(metal))

def N_sp(metal):
    n_dw = Dosplot('tetra-{}N2/PDOS_N_DW.dat'.format(metal))
    n_dw.s_plot()
    n_dw.p_plot()
    n_dw.tot_plot(label='')
    n_up = Dosplot('tetra-{}N2/PDOS_N_UP.dat'.format(metal))
    n_up.s_plot()
    n_up.p_plot()
...
def mdn2p(metal):
    ...
    n_dw = Dosplot('tetra-{}N2/PDOS_N_DW.dat'.format(metal))
    n_dw.p_plot(p='$N_{2p}$')
    n_dw.tot_plot(color='blueviolet',label='')
    ...
    m_dw = Dosplot('tetra-{}N2/PDOS_{}_DW.dat'.format(metal, metal))
    m_dw.d_plot(d=metal+'${}_{d}$')
    m_dw.tot_plot(color='darkred',label='')
...
if __name__=="__main__":
    ...
    for i in metals:
        ...
        plt.title('tetra-'+i+'$N_2$',fontsize=20)
        plt.tick_params(direction='in',labelsize=15)
        plt.xlim(-2,2)
        plt.ylim(-1.2,1.2)
        plt.savefig('tetra-{}N_2'.format(i), figsize=(6, 2), dpi=300, bbox_inches='tight')
        # plt.show()
