...
class E1C2d():
    ...
    def E1vbm(self):
        ...
        plt.plot(self.strain,self.vbm,label='Strain-VBM')
        plt.plot(self.strain,e1v_fit,label='Strain-VBM-fit')
        plt.legend()
        plt.savefig('Strain-VBM')
        return e1v[0]
    ...
    def E1cbm(self):
        ...
        print(E1c)
        fig = plt.figure(figsize=(4, 3), dpi=300)
        plt.plot(self.strain,self.cbm,label='Strain-CBM')
        plt.plot(self.strain,e1c_fit,label='Strain-CBM-fit')
        plt.legend()
        plt.savefig('Strain-CBM')
        return e1c[0]
    ...
    def C2d(self):
        ...
        fig = plt.figure(figsize=(4, 3), dpi=300)
        plt.plot(self.strain, self.energy, label='Strain-Energy')
        plt.plot(self.strain, c2d_fit, label='Strain-Energy-fit')
        plt.legend()
...
def mstar(mstar):
    with open(mstar,'r') as f:
        mstar=f.readlines()
        m_v_y=float(mstar[1].split()[1])
        m_v_x=float(mstar[1].split()[2])
        m_c_y=float(mstar[2].split()[1])
        m_c_x=float(mstar[2].split()[2])
...
if __name__=="__main__":
    ...
    mobility_y_vbm = miu(C2d_x, m_cbm_y, m_cbm_x, E1v_x)
    print("Miu_vbm_x: {}".format(mobility_x_vbm))
    print("Miu_vbm_y: {}".format(mobility_x_vbm))
    print(30 * '-'+"About CBM(Electron)"+30 * '-')
    E1c_y=y.E1cbm()
    E1c_x=x.E1cbm()
    mobility_x_cbm=miu(C2d_x,m_cbm_x,m_cbm_y,E1c_x)
    mobility_y_cbm = miu(C2d_x, m_cbm_y, m_cbm_x, E1c_x)
    print("Miu_cbm_x: {}".format(mobility_x_cbm))
    print("Miu_cbm_y: {}".format(mobility_x_cbm))
    print(30 * '-' + "Results summary" + 30 * '-')
    print("Y direction elasticity modulus C2d_y is: {:.2f} J/m^2".format(C2d_y))
    print("X direction elasticity modulus C2d_x is: {:.2f} J/m^2".format(C2d_x))
    print(20 * '-' + "CBM(electron) summary" + 20 * '-')
    print("m*_x_cbm: {:.2f} m0".format(m_cbm_x))
    print("m*_y_cbm: {:.2f} m0".format(m_cbm_y))
    print("E1_x_cbm: {:.2f} eV".format(E1c_x))
    print("E1_y_cbm: {:.2f} eV".format(E1c_y))
    print("Mobility_x_cbm: {:.2f} cm2/s/v".format(mobility_x_cbm))
    print("Mobility_y_cbm: {:.2f} cm2/s/v".format(mobility_y_cbm))
    print(20 * '-' + "VBM(hole) summary" + 20 * '-')
    print("m*_x_vbm: {:.2f} m0".format(m_vbm_x))
    print("m*_y_vbm: {:.2f} m0".format(m_vbm_x))
    print("E1_x_vbm: {:.2f} eV".format(E1v_x))
    print("E1_y_vbm: {:.2f} eV".format(E1v_y))
    print("Mobility_x_vbm: {:.2f} cm2/s/v".format(mobility_x_vbm))
    print("Mobility_y_vbm: {:.2f} cm2/s/v".format(mobility_y_vbm))